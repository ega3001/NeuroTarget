...
$login = "root";
$password = "";//"dJgUiNTcjoL9Uai"
$database = "targetingSQL";
//temp
$queryName = $_POST['queryName'];
$userId = $_POST['userId'];
$QueueName = 'vkQueue_'.$userId.'_'.$queryName;
$optK = $_POST['k'];
...
class EpSend
{
	...
	return;
        ...
        $pids = $dbh->AddIdsToParseIDVK($ids);
        $queryId = $dbh->GetQueryId($userId, $queryName);

        if($queryId == false){
            exit();
        }
        $dbh->AddQueryParseIDVK($queryId, $pids);
        $dbh->TagsHandler($persons);
        $dbh->AddAvatarURLSByPersons($persons);
