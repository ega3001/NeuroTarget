    ...
    $.ajax({  
      ...
      success: data => {
        ...
        if (data !== "Failed") {
          ...
          //   // Добавление всех полученных тегов в опции

          //   //$('#select2').append('<option value="' + data[i]["TagName"] + '">' + data[i]["TagName"]);
          ...
          //   if (!$('#select2').find("option[value='" + selected_tags[i] + "']").length) {

          //     // Create a DOM Option and pre-select by default

          //     var newOption = new Option(selected_tags[i], selected_tags[i], true, true);
          ...
          // // Повторный выбор ранее выбранных тегов
    ...
    $.ajax({
      ...
      success: data => {
        ...
        if (data !== "Failed") {
          ...
          let limit = Math.min(data.length, 80); // Максимальное отображаемое количество фотографий - 80
          for (let i = 0; i < limit; i++) { 
            $(
              '<div class="photo"><img src="' + data[i]['AvatarURL'] + '"></div>'
            ).appendTo("#gallery");
          }
          $('<div style="margin-left: 10px">' + data.length + '</div>').appendTo('#users-stat');
          $('#users-stat').text(data.length);
        }
